...
async function inspectViewport(browserInstance, testCase) {
  ...
  await page.route("**/assets/**", async (route) => {
    ...
    if (requestUrl.origin !== FRONTEND_ORIGIN) {
      await route.continue();
      return;
    }
    await route.fulfill({ path: `.output/public${requestUrl.pathname}` });
  });
  ...
  try {
    ...
    await page
      .getByRole("heading", { name: testCase.stageHeading, exact: true })
      .waitFor();
    await page
    ...
    const metrics = await page.evaluate(() => {
      ...
      return {
        ...
        canvas: toPlainRect(canvasRect),
        contextRail: toPlainRect(railRect),
      };
